	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		public String Get_Minimum_Time(String src, String dst) {
		    ...
		    while (!stack.isEmpty()) {
		        ...
		        // if there exists a direct edge between the removed pair and destination vertex
		        if (rp.vname.equals(dst)) {
		        ...
		        for (String nbr : nbrs) {
		            ...
		            if (!processed.containsKey(nbr)) {
		                ...
		                np.psf = rp.psf + nbr + "  ";
		                np.min_dis = rp.min_dis + rpvtx.nbrs.get(nbr);
		                np.min_time = rp.min_time + 2 + rpvtx.nbrs.get(nbr); // Consider transit time between stations (e.g., 2 minutes)
		                stack.addFirst(np);
		            }
		        }
		    }
		...
		public double calculateFare(String src, String dest, boolean byTime) {
	        if (byTime) {
	            ...
	            double fare = minutes * 2.5; // Adjust the fare calculation as per your requirements
	            return fare;
	        } else {
	            ...
	            double fare = distance * 10; // Adjust the fare calculation as per your requirements
	            return fare;
	        }
	    }
			...
			while (true) {
				...
				case 3:
					...
					System.out.println("SHORTEST DISTANCE FROM "+st1+" TO "+st2+" IS "+g.dijkstra(st1, st2, false)+"KM\n");
					  double distanceFare = g.calculateFare(st1, st2, false);
	                    System.out.println("Fare based on distance: " + distanceFare + " INR");
					break;
				...
				case 4:
					...
					System.out.println("SHORTEST TIME FROM ("+sat1+") TO ("+sat2+") IS "+g.dijkstra(sat1, sat2, true)/60+" MINUTES\n\n");
					 double timeFare = g.calculateFare(sat1, sat2, true);
	                    System.out.println("Fare based on time: " + timeFare + " INR");
	                    break;
				...
				case 7:
                    ...
                    System.out.print("1. Calculate Fare by Distance\n2. Calculate Fare by Time\nEnter your choice: ");
                    int fareChoice = Integer.parseInt(inp.readLine());
                    
                    if (fareChoice == 1) {
                        double distanceFare1 = g.calculateFare(sourceStation, destinationStation, false);
                        System.out.println("Fare based on distance: " + distanceFare1 + " INR");
                    } else if (fareChoice == 2) {
                        double timeFare1 = g.calculateFare(sourceStation, destinationStation, true);
                        System.out.println("Fare based on time: " + timeFare1 + " INR");
                    } else {
